...
const symWinKey = Symbol("winKey");
const symKeyType = Symbol("keyType");
...
class VConsoleDefaultTab extends VConsoleLogTab {
  ...
  protected async updateWinKeys() {
    // do not traverse these keys to prevent "Deprecation" warning
    const keyBlackList = ["webkitStorageInfo"];

    const winKeys = (
      await this.evaluate("globalThis").then((res) =>
        this.getObjOwnPropertyNames(res)
      )
    ).sort();
    let keyTypes = {};
    ...
    Reflect.set(globalThis, symWinKey, winKeys);
    Reflect.set(globalThis, symKeyType, keyTypes);
  }

  protected getWinKeys() {
    const winKeys = Reflect.get(globalThis, symWinKey) ?? [];
    const keyTypes = Reflect.get(globalThis, symKeyType) ?? {};
    return { winKeys, keyTypes };
  }
  ...
  onReady() {
    ...
    const retrievePrecedingIdentifier = (
      text: string,
      pos: number,
    ...
    $.bind($input, "keyup", async function (e) {
      ...
      if (value && value.length > 0) {
        ...
        console.debug(value);
        if ("." === value) {
          const key = retrievePrecedingIdentifier(
            tempValue,
            tempValue.length - 1
          );
          ...
          if (!cacheObj[key]) {
            try {
              const result = await that.evaluate(key);
              cacheObj[key] = (
                await that.getObjOwnPropertyNames(result)
              ).sort();
            } catch (e) {}
          }
          try {
          ...
          const { winKeys, keyTypes } = that.getWinKeys();
          for (let i = 0; i < winKeys.length; i++) {
    ...
    $.bind($.one(".vc-cmd.vc-filter", this.$tabbox), "submit", function (e) {
      ...
      let $cmdInput = <HTMLInputElement>(
        $.one(".vc-cmd.vc-filter .vc-cmd-input", e.target)
      );
  ...
  beforeRenderLog($line: Element) {
    this.checkFilterInLine($line)
      ? $.addClass($line, "hide")
      : $.removeClass($line, "hide");
  }
  ...
  protected async getObjOwnPropertyNames(obj) {
    return Object.getOwnPropertyNames(obj);
  }

  protected async evaluate(cmd: string): Promise<any> {
    try {
      return eval.call(window, "(" + cmd + ")");
    } catch (e) {
      try {
        return eval.call(window, cmd);
      } catch (e) {}
   ...
   * Run a command
   */
  private async evalCommand(cmd: string) {
    ...
    let result = (await this.evaluate(cmd)) || void 0;
    ...
    await this.updateWinKeys();
  }

  private checkFilterInLine($dom: Element) {
    return (
      $dom.innerHTML.toUpperCase().indexOf(this.filterText.toUpperCase()) === -1
    );
  }
} // END class
